                ...
                <div class="col-md-8">
                    @foreach($articles as $ac)
                        <div class="d-help-blog-list listing">
                            <a href="{{ url('detail',['id'=>$ac->id]) }}">
                                {{ str_limit($ac->title,60,'...') }}
                                <span class="pull-right" style="color: #c0c0c0;">{{ $ac->time }}   <i class="fa fa-heart"></i>{{ $ac->brows }}</span>
                            </a>
                        </div>
                    @endforeach
                </div>
